...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{ // 상속
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
        ...
        QueryResults<MainItemDto> results = queryFactory
                .select(
                        new QMainItemDto( // QMainItemDto 의 생성자에 반황할 값들을 넣어준다. @QueryProjection 을 사용하면 Dto로 바로 조회가 가능하다
                                item.id,  // 엔티티 조회 후 DTO 반환하는 과정을 줄일 수 있다.
                                item.itemNm,
                                item.itemDetail,
                                itemImg.imgUrl,
                                item.price)
                ...
                .from(itemImg)
                .join(itemImg.item, item) // itemImg 와 item 내부를 조인한다.
