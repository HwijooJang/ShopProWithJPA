...
public class OrderService {
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable){
        ...
        for (Order order : orders){ // 주문 리스트를 순회하면서 구매 이력 페이지에 전달한 DTO를 생성한다.
            OrderHistDto orderHistDto = new OrderHistDto(order);
            List<OrderItem> orderItems = order.getOrderItems();
            for(OrderItem orderItem : orderItems){
                ItemImg itemImg = itemImgRepository.findByItemIdAndRepimgYn(orderItem.getItem().getId(), "Y"); // 주문한 상품의 대표이미지를 조회
    ...
    // 주문을 취소하는 로직
    @Transactional(readOnly = true)
    public boolean validateOrder(Long orderId, String email){ // 현재 로그인한 사용자와 주문 데이터를 생성한 사용자가 같은지 검사를 한다.
        Member curMember = memberRepository.findByEmail(email); // 같을 때는 true 를 반환하고 같지 않을 경우 false 를 반환한다
    ...
    public void cancelOrder(Long orderId){
        Order order = orderRepository.findById(orderId).orElseThrow(EntityNotFoundException::new);
        order.cancelOrder(); // 주문 취소 상태로 변경하면 변경 감지에 의해서 트랜잭션이 끝날 때 update 쿼리가 실행된다.
    }
    // 장바구니에서 주문할 상품 데이터를 전달 받아 주문을 생성하는 로직을 만든다.
